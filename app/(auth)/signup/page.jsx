...
import { motion } from "framer-motion";
import { signIn } from "next-auth/react";
import { ArrowRight, CheckCircle, TrendingUp, Users, Edit3 } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const StatusMessage = ({ type, message }) => {
    ...
    return (
      <motion.div
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        className={`p-4 rounded-2xl border ${styles[type]} mb-4`}
      >
        {message}
      </motion.div>
    );
  ...
  const FormInput = ({ id, label, type, placeholder, required, minLength }) => (
    <div className="mb-4">
      <label
        htmlFor={id}
        className="block text-sm font-semibold text-gray-700 mb-2"
    ...
    >
      <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-8 sm:p-10 space-y-6 border border-white/20 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <StatusMessage type="error" message={status.error} />
          <StatusMessage type="verified" message={status.verified} />
